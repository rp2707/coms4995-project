...
def testGAN(trained_model_path=None, n_batches=2):
    ...
    with tf.Session() as sess:
        ...
        for j in range(n_batches):
            ...
            for i in range(4):
                ...
                savename = 'save-{0}-{1}_16K'.format(j, i)
                pickle.dump(g_objects, open('{0}.p'.format(savename), 'wb'))
                np.save('{}_zvectors'.format(savename),z_sample)
                #if g_objects[id_ch[i]].max() > 0.5:
                #    d.plotVoxelVisdom(np.squeeze(g_objects[id_ch[i]]>0.5), vis, '_'.join(map(str,[i])))
